...
CMD ["catalina.sh", "run"]
ENV TZ Europe/Kiev
HEALTHCHECK --interval=10s --timeout=3s --retries=6 CMD curl -f http://app:80/web_ejb/ \
|| exit 1
HEALTHCHECK --interval=10s --timeout=3s --retries=12 CMD curl -f http://app:80/web_ejb/JNDIServlet \
|| exit 1
#EXPOSE 8080